...
> Tài liệu này hướng dẫn từng bước để cài đặt và chạy dự án **Web CodeSoChill** trên máy tính cá nhân. Hãy thực hiện theo đúng thứ tự từ trên xuống dưới để tránh gặp lỗi không mong muốn.
> 
...
User database là tài khoản để code của bạn có quyền đọc/ghi dữ liệu vào MongoDB. Đây khác với tài khoản đăng nhập Atlas.
...
Thay `<username>` và `<password>` bằng thông tin user vừa tạo ở Bước 3. Lưu chuỗi này lại để dùng trong file `.env` của backend.

---

### Bước 6: Kiểm tra kết nối (Tuỳ chọn)

Nếu muốn xác nhận kết nối hoạt động trước khi bắt đầu code, bạn có thể dùng **MongoDB Compass** – ứng dụng GUI để xem và quản lý database.
...
4. Tạo database mới cho dự án: Click **"Create Database"** → Đặt tên `codesochill`.
...
```bash
git clone https://github.com/your-username/WebCodeSoChill.git
cd WebCodeSoChill/backend
```
...
Nếu bạn đang khởi tạo backend từ đầu (chưa có `package.json`), chạy lần lượt:
...
Giải thích vai trò từng package:
...
```
MONGOOSE_DB_URL=mongodb+srv://<username>:<password>@<cluster>.mongodb.net/codesochill?retryWrites=true&w=majority
PORT=5000
JWT_SECRET=your_jwt_secret_key_here
```

Thay các giá trị trong `< >` bằng thông tin thực của bạn từ bước cài MongoDB Atlas ở trên.
