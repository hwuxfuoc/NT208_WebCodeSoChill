...
/** Detect if viewport is in "mobile" modal mode (≤900px) */
...
export default function MessagesModal() {
  ...
  // "list" = show conversation list; "chat" = show chat window (mobile only)
    ...
    const loadConvos = async () => {
      try {
        ...
        if (fetchedConvos.length > 0) {
          ...
          // On desktop open first chat automatically
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) setSearchResults(res?.data?.users || []);
      } catch (err: any) {
  ...
  async function handleSelectUser(user: any) {
    ...
    try {
      ...
      setConvos((prev) => {
        ...
        if (exists) return prev.map((c) => (String(c.id) === String(conversation.id) ? conversation : c));
        return [conversation, ...prev];
      });
      setActive(String(conversation.id));
      setMobileView("chat"); // switch to chat view on mobile after selecting
    } catch (err: any) {
  ...
  function handleConvoSelect(convId: string) {
    setActive(convId);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setConvos((prev) => prev.map((c) => {
        ...
        return { ...c, last: newMessage.text, time: newMessage.time };
      }));
    } catch (err: any) {
  ...
  // On mobile: show list OR chat based on mobileView state
  ...
  return (
    <motion.div
      className="modal-panel messages-modal w-[780px] h-[520px] p-0 flex overflow-hidden border-none shadow-2xl relative"
    ...
    >
      ...
      <button
        onClick={closeModal}
        className="absolute top-3 right-3 w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-400 transition-colors z-20"
        aria-label="Close"
      >
      ...
      {showList && (
        <div className="messages-modal-sidebar w-[280px] border-r border-gray-100 flex flex-col bg-white">
          <div className="px-5 pt-5 pb-3 flex items-center gap-2 pr-12">
            <h2 className="text-base font-extrabold text-[#1A1D2B] flex-1">Messages</h2>
            {/* Compose icon */}
          ...
          <div className="flex-1 overflow-y-auto">
            {searchTerm.trim().length >= 2 ? (
              <div className="space-y-2 px-3">
                {searchLoading ? (
                  <div className="text-sm text-gray-500 py-3 px-1">Đang tìm kiếm...</div>
                ) : searchResults.length === 0 ? (
                  <div className="text-sm text-gray-500 py-3 px-1">Không tìm thấy user nào.</div>
                ) : (
                  searchResults.map((user) => (
                    <button
                      key={user._id || user.username}
                      onClick={() => handleSelectUser(user)}
                      className="w-full text-left rounded-2xl border border-gray-100 px-3 py-3 hover:bg-gray-50 transition-colors"
                    >
              ...
              convos.map((c) => (
                <button
                  key={c.id}
                  onClick={() => handleConvoSelect(String(c.id))}
                  className={`w-full flex items-center gap-3 px-4 py-3 border-l-4 transition-colors text-left ${String(active) === String(c.id) && !isMobile ? "bg-orange-50/60 border-l-[var(--main-orange-color)]" : "border-l-transparent hover:bg-gray-50"}`}
                >
                  <div className="relative shrink-0">
                    <img src={c.avatar || DEFAULT_AVATAR} alt={c.name} className="w-9 h-9 rounded-full" />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-[11px] truncate ${String(active) === String(c.id) && !isMobile ? "text-orange-500 font-semibold" : "text-gray-400"}`}>{c.last}</p>
                  </div>
                </button>
              ))
            )}
      ...
      {/* ── CHAT AREA ── */}
      {showChat && (
        <div className="messages-modal-chat flex-1 flex flex-col bg-[#fafbfc]">
          {/* Chat header */}
          <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 bg-white pr-12">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold" style={{ color: convo?.online ? "#22c55e" : "#9ca3af" }}>
                {convo ? (convo.online ? "Online Now" : "Offline") : "Chưa có cuộc hội thoại"}
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
            ...
            {messages.map((m) => (
              <div key={m.id} className={`flex items-end gap-2 max-w-[78%] ${m.me ? "ml-auto flex-row-reverse" : ""}`}>
                ...
                <div
                  className={`px-4 py-2.5 rounded-2xl ${m.me ? "rounded-br-none" : "rounded-bl-none"}`}
