...
import { todayChallenges } from "../../utils/mockData";
...
export default function TodayChallengePanel() {
  ...
  const { solvedCount, total, exp, problems } = todayChallenges;

  const pairIndex = Math.min(Math.floor(solvedCount / 2) * 2, total - 2);
  const currentPair = problems.slice(pairIndex, pairIndex + 2);
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-sm font-semibold opacity-90">Solve {total} problems · earn {exp} EXP</p>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-semibold group-hover:underline">{p.title}</span>
                </div>
                <span
